...
def has_kitty_gene(dna):
    ...
    False
    """
    return has_sublist(dna,link('C',link('A',link('T',link('C',link('A'))))))
...
def count_change(amount, denominations):
    ...
    if amount == 0:
        return True
    andy = count_change(amount - first(denominations),denominations)
    reid = count_change(amount,rest(denominations))
    return andy + reid
